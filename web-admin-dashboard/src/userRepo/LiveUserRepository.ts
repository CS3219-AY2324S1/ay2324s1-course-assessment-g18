...
class LiveQuestionRepository {
    ...
    async updateUser(username: string, userEmail: string, userRole: UserRole, id: number): Promise<User> {
        const res = await axios.put(`/users/update/${userEmail}`, {
            username,
            userEmail,
            userRole,
        }, this.config);
    ...
    async addUser(username: string, userEmail: string, userRole: UserRole): Promise<User> {
